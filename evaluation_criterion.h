#ifndef EVALUATION_CRITERION_H
#define EVALUATION_CRITERION_H
...
//constexpr int inf_ = 0x3f3f3f3f;
//constexpr int MaxN_ = 4096;
...
//    void CirleFit(const QVector<cv::Point2f>& points, const QVector<double>& weights, cv::Point2f& circleCenter, double& radius);
    ...
    void leastSquaresMeanCircle(QVector<eva_radius_x_y> &points);           //最小二乘拟合圆
    ...
    double getDistanceSquare(const eva_radius_x_y a, const eva_radius_x_y b);

    eva_radius_x_y getCircleCenter(const eva_radius_x_y a, const eva_radius_x_y b, const eva_radius_x_y c);

    void minCoverCircle(QVector<eva_radius_x_y> p, int pointCount, eva_radius_x_y& center, double& radius);//找最小覆盖圆(这里没有用全局变量p[], 因为是为了封装一个函数便于调用)
...
public:
    ...
    eva_radius_x_y radius_x_y_;
    QVector<eva_radius_x_y> point_info_;

    eva_radius_x_y tempPoint_;        //定义临时变量，存储最小二乘圆心坐标

    eva_radius_x_y min_coverage_circle_;
